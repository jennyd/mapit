...
class Command(BaseCommand):
    ...
    def handle(self, kml_path, names_path, **options):
        ...
        # Data for neighbourhoods in Northern Ireland was recently added to the
        # API (between 31/07/2012 and 16/08/2012), but no KMLs are available yet,
        # so we are ignoring these for now. log_extra_names doesn't log these
        # names either.
        names_ignored = ('forces.json',
                         'northern-ireland_neighbourhoods.json',
                         'README')

        kml_forces_list = [force for force in os.listdir(kml_path) if force not in kml_ignored]
        names_forces_list = [force for force in os.listdir(names_path) if force not in names_ignored]

        if len(kml_forces_list) != len(names_forces_list):
